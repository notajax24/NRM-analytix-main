// src/pages/TalentJobAnalyzerPage.tsx
...
import Solution4Img from '../assets/solution/solution4.png';
...
export default function TalentJobAnalyzerPage() {
  useEffect(() => { document.title = "Dev_Sync | NRM Analytix"; }, []);
        ...
        <Container maxWidth="lg">
          ...
          <FadeUp delay={300}>
            <Card elevation={0} sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: 'stretch', gap: { xs: 3, md: 6 }, px: { xs: 3, md: 5 }, py: { xs: 2.5, md: 4 }, mb: { xs: 6, md: 8 }, borderRadius: '24px', border: '1px solid #f1f5f9', bgcolor: '#ffffff', boxShadow: '0 20px 40px rgba(0,0,0,0.04)' }}>
              ...
              <Box sx={{ flex: 1.2, width: '100%', minHeight: { xs: 240, md: 340 }, maxHeight: { md: 360 }, height: { xs: 'auto', md: '100%' } }}>
                <Box component="img" src={Solution4Img} alt="Talent Match Metrics Display" sx={{ width: '80%', height: '100%', objectFit: 'cover', borderRadius: '24px', display: 'block' }} />
              </Box>
            </Card>
          </FadeUp>
            ...
            <FadeUp delay={400}>
              <Card elevation={0} sx={{ height: '100%', p: { xs: 3, sm: 4, md: 5 }, bgcolor: 'rgba(250, 245, 255, 0.5)', borderRadius: '16px', border: '1px solid #d8b4fe' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                  ...
                  <Typography variant="h5" sx={{ color: '#ef4444', fontWeight: 300, letterSpacing: 1, fontSize: { xs: '1.2rem', md: '1.5rem' } }}>THE OLD WAY</Typography>
                </Box>
                <Typography variant="body1" sx={{ color: '#ef4444', lineHeight: 1.6, fontSize: { xs: '0.95rem', md: '1.05rem' }, fontWeight: 300 }}>
                  Write code → break → debug → repeat. Manual SDK calls, mismatched configs, tribal knowledge required at every step.
                </Typography>
              </Card>
            </FadeUp>
            <FadeUp delay={500}>
              <Card elevation={0} sx={{ height: '100%', p: { xs: 3, sm: 4, md: 5 }, bgcolor: 'rgba(254, 242, 242, 0.5)', borderRadius: '16px', border: '1px solid #fca5a5' }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
                  ...
                  <Typography variant="h5" sx={{ color: '#5b21b6', fontWeight: 300, letterSpacing: 1, fontSize: { xs: '1.2rem', md: '1.5rem' } }}>THE DEV_SYNC WAY</Typography>
                </Box>
                <Typography variant="body1" sx={{ color: '#5b21b6', lineHeight: 1.6, fontSize: { xs: '0.95rem', md: '1.05rem' }, fontWeight: 300 }}>
                 Say what you need. dev_sync reads the right patterns, calls the right tools, and deploys — automatically.
                </Typography>
              </Card>
            </FadeUp>
          </Box>
          ...
          <Box sx={{ mb: { xs: 8, md: 12 } }}>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: { xs: 2, sm: 3 } }}>
              {[
                { title: '50+ Executable Tools', desc: 'SQL, Jobs, Pipelines, Dashboards, Model Serving — every Databricks operation, one AI command away.' },
                { title: '19+ Built-in Skills', desc: 'Best practices and proven patterns baked in. Your AI already knows how Databricks should be built.' },
                { title: 'Python Library', desc: 'High-level APIs that abstract the SDK complexity. Zero boilerplate, type-safe, serverless by default.' },
                { title: 'Full Web Application', desc: 'Complete UI for non-technical stakeholders. Claude-powered chat that builds on Databricks for you.' }
              ].map((cap, i) => (
                <Box key={i}>
          ...
          <Box sx={{ mb: { xs: 8, md: 14 } }}>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', md: 'repeat(4, minmax(0, 1fr))' }, gap: { xs: 2, sm: 3 } }}>
              {[
                { value: '10–50x', label: 'Faster Development', color: 'linear-gradient(135deg, #ef4444 0%, #f97316 100%)', bg: 'radial-gradient(circle at 50% -20%, rgba(254, 226, 226, 0.8) 0%, #ffffff 70%)' },
                { value: '5 mins', label: 'Setup to First Deploy', color: 'linear-gradient(135deg, #8b5cf6 0%, #3b82f6 100%)', bg: 'radial-gradient(circle at 50% 120%, rgba(243, 232, 255, 0.8) 0%, #ffffff 70%)' },
                { value: '50+', label: 'AI-Powered\n Tools', color: 'linear-gradient(135deg, #ef4444 0%, #f97316 100%)', bg: 'radial-gradient(circle at 50% 120%, rgba(254, 226, 226, 0.8) 0%, #ffffff 70%)' },
                { value: '0', label: 'Lines of\n Boilerplate', color: 'linear-gradient(135deg, #8b5cf6 0%, #3b82f6 100%)', bg: 'radial-gradient(circle at 50% -20%, rgba(243, 232, 255, 0.8) 0%, #ffffff 70%)' }
              ].map((impact, i) => (
                <Box key={i}>
                  <FadeUp delay={300 + (i * 100)}>
